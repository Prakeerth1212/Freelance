...
public class PdfService {
    ...
    public PdfService(InvoiceDAO invoiceDAO, ProjectDAO projectDAO,
                      ClientDAO clientDAO, TimeLogDAO timeLogDAO) {
    ...
    public String generateInvoice(int invoiceId) throws IOException {
        Invoice invoice = invoiceDAO.getById(invoiceId)
                .orElseThrow(() -> new IllegalArgumentException("Invoice not found: " + invoiceId));

        Project project = projectDAO.getById(invoice.getProjectId())
                .orElseThrow(() -> new IllegalArgumentException("Project not found for invoice: " + invoiceId));

        Client client = clientDAO.getById(project.getClientId()).orElse(null);
    ...
    private void addTimeLogTable(Document document, List<TimeLog> timeLogs, Project project) {
        ...
        for (String h : headers) {
            Cell headerCell = new Cell().add(new Paragraph(h).setBold().setFontSize(10).setFontColor(ColorConstants.WHITE))
